...
object HuaixinXiaokuHuisuHealthETL {
...
//    es.batch.size.entries, "10000"
    ...
    val value1 = EsSpark.esJsonRDD(sc, indexType, qurey, ES_READ_CONFIG).map(t => {
      ...
      val dataObject = parseObject(t._2)
...
//        industry_keywords //行业信息-常规关键字打标   0：不符合关键词提取规则 1：符合关键词提取规则
       ...
       dataObject
    }).filter(t => t.getIntValue("garbage_classify") != 1 && StringUtils.isNotBlank(t.getString("health_level")) && !t.getString("health_level").equals("0"))
      .map(f = t => {
        t.toJSONString
    })
      .repartition(1).saveAsTextFile("/fengkong/resources/huaixinHealthETL")
...
//        EsSpark.saveToEsWithMeta(value1,indexType)
   ...
   class Result(title:String,content:String,health_level:String,health_level_subsection:String){
   }
